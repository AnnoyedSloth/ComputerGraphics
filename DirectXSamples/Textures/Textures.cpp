//-----------------------------------------------------------------------------
// File: Textures0000888.cpp
//
// Desc: Better than just lights and materials, 3D objects look much more
//       convincing when texture-mapped. Textures0000888 can be thought of as a sort
//       of wallpaper, that is shrinkwrapped to fit a texture. Textures0000888 are
//       typically loaded from image files, and D3DX provides a utility to
//       function to do this for us. Like a vertex buffer, Textures0000888 have
//       Lock() and Unlock() functions to access (read or write) the image
//       data. Textures0000888 have a width, height, miplevel, and pixel format. The
//       miplevel is for "mipmapped" Textures0000888, an advanced performance-
...
//
//       Textures0000888 are associated with geometry through texture coordinates.
...
//-----------------------------------------------------------------------------
// Name: InitGeometry()
// Desc: Create the Textures0000888 and vertex buffers
//-----------------------------------------------------------------------------
HRESULT InitGeometry()
{
	// Use D3DX to create a texture from a file based image
	D3DXCreateTextureFromFile(g_pd3dDevice, L"sonic1.png", &g_pTexture);
	D3DXCreateTextureFromFile(g_pd3dDevice, L"sonic2.png", &g_pTexture1);
	D3DXCreateTextureFromFile(g_pd3dDevice, L"sonic3.png", &g_pTexture2);
	D3DXCreateTextureFromFile(g_pd3dDevice, L"sonic4.png", &g_pTexture3);
	...
	if (FAILED(g_pVB->Lock(0, 0, (void**)&pVertices, 0)))
		return E_FAIL;
	pVertices[0].position = D3DXVECTOR3(0.0, 0.0f, 0.0f);
	pVertices[0].color = 0xffffffff;
	pVertices[0].tu = 0.0f;
	pVertices[0].tv = 0.0f;

	pVertices[1].position = D3DXVECTOR3(1.0, 0.0f, 0.0f);
	pVertices[1].color = 0xffffffff;
	pVertices[1].tu = 1.0f;
	pVertices[1].tv = 0.0f;

	pVertices[2].position = D3DXVECTOR3(0.0, 1.0f, 0.0f);
	pVertices[2].color = 0xffffffff;
	pVertices[2].tu = 0.0f;
	pVertices[2].tv = 1.0f;

	pVertices[3].position = D3DXVECTOR3(1.0, 1.0f, 0.0f);
	pVertices[3].color = 0xffffffff;
	pVertices[3].tu = 1.0f;
	pVertices[3].tv = 1.0f;

	pVertices[4].position = D3DXVECTOR3(1.0, 0.0f, 0.0f);
	pVertices[4].color = 0xffffffff;
	pVertices[4].tu = 1.0f;
	pVertices[4].tv = 0.0f;

	pVertices[5].position = D3DXVECTOR3(0.0, 1.0f, 0.0f);
...
INT WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, INT)
{
	...
	HWND hWnd = CreateWindow(L"D3D Tutorial", L"D3D Tutorial 05: Textures0000888",
